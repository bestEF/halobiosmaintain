...
public class ParseXMLUtil {
    ...
    public ParseXMLUtil(File xmlFilePath){
        ...
        try {
            ...
            /**测试entityMap 是否正确**/
//			Map enMap = (Map) this.getEntityMap().get("Sheet1");
...
//			}
            ...
            /*			*//**测试column list是否正确**/
//			List colList = (List) this.getColumnListMap().get("金属信息表");
//			System.out.println("column size:"+colList.size());

            /**测试columnMap是否正确**/
//			Map colMap = (Map) this.getColumnMap().get("金属信息表_金属类型");
...
//			    List rulesValidList = (List) this.getColumnRulesMap().get("金属信息表_金属类型");
//				for(int i=0;i<rulesValidList.size(); i++){
//					Map colRuleMap = (Map) rulesValidList.get(i);
//				    String ruleName = (String) colRuleMap.get("name");
//				    Map ruleMa = (Map) this.getRuleMap().get("金属信息表_金属类型_"+ruleName); //eg: 用户表_用户名_nullable
...
//			}
    ...
    public void setRuleMap(String entityName,String columnName,Element ruleValid){
        if(ruleValid != null){
            ...
            String nullable = ruleValid.attributeValue("nullable");
            String length = ruleValid.attributeValue("length");
            String allowLine = ruleValid.attributeValue("allowLine");
            String lower = ruleValid.attributeValue("lower");
            String upper = ruleValid.attributeValue("upper");
            Map ruleValidMap = new HashMap();
            ruleValidMap.put("name", ruleName);
            ruleValidMap.put("message", ruleMsg);
            if(ruleName.equals("nullable")) {
                ruleValidMap.put("nullable", nullable);
            }else if(ruleName.equals("checkRange")) {
                ruleValidMap.put("lower", lower);
                ruleValidMap.put("upper", upper);
            }else if(ruleName.equals("length")) {
                ruleValidMap.put("length", length);
            }else if(ruleName.equals("allowLine")) {
                ruleValidMap.put("allowLine", allowLine);
            }
    ...
    public static void main(String[] args) {
        File file = new File("src/main/java/com/ltmap/platform/manager/dataManager/materialData/metal/model/MetalInfo.xml");
